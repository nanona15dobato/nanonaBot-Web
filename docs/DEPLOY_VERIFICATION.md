...
### 11.7 フェーズ7: Template:リダイレクトの所属カテゴリの自動編集

`利用者:Nanona15dobato/sandbox`のような単一ページでは`{{リダイレクトの所属カテゴリ}}`の
実運用を再現しにくいため、この節は既にこのテンプレートが使われている実際の記事を
対象にしたCategory置換／除去タスクを作る際の確認項目とする（対象カテゴリのメンバー数が
少なく、影響範囲を把握できるものを選ぶこと）。

1. [ ] 対象カテゴリに`{{リダイレクトの所属カテゴリ}}`で当該カテゴリ名を保持しているページが
   含まれることを事前に手動確認しておく（`https://ja.wikipedia.org/w/index.php?search=insource:%22リダイレクトの所属カテゴリ%22...`等）
2. [ ] Category置換タスクを作成し、タスク詳細画面のページ一覧に、カテゴリメンバーの記事に加えて
   `{{リダイレクトの所属カテゴリ}}`で言及しているだけの記事（カテゴリタグ自体は無い）も
   対象として含まれていることを確認
3. [ ] 該当ページのDiffで、`[[Category:...]]`タグと`{{リダイレクトの所属カテゴリ}}`内の
   該当引数の**両方**が書き換え対象になっていることを確認
4. [ ] 承認後、実際に両方が正しく書き換わっていることをWikipedia側で確認
5. [ ] 警告パネルには、構造的に確認できたページは表示されず、緩い一致のみが
   残っていることを確認（表示されている場合は内容を見て妥当か判断する）
